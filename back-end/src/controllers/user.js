...
controller.login = async function (req, res) {
  try {
    ...
    // Determinamos o nível de atraso atual.
    let currentDelayLevel;

    // Se o nível de atraso armazenado no usuário estiver no vetor
    // delayLevels, usamos este nível
    if (delayLevels.length < user.delay_level)
      currentDelayLevel = user.delay_level;
    // Senão, usamos o nível mais alto disponível
    else currentDelayLevel = delayLevels.length - 1;

    // Usuário encontrado, precisamos verificar se ele ainda tem
    // tentativas de login disponíveis
    let lastLoginAttempt;
    if (user.last_attempt)
      lastLoginAttempt = new Date(Date.parse(user.last_attempt));
    else lastLoginAttempt = new Date();

    const now = new Date();
    const attemptsAllowedAter = new Date(
      lastLoginAttempt.getMinute() + delayLevels[currentDelayLevel]
    );

    if (
      user.login_attempts >= Number(process.env.MAX_LOGIN_ATTEMPTS) &&
      now <= attemptsAllowedAter
    ) {
      // Avisa que o usuário poderá tentar de novo após XXX milissegundos
      res.setHeader("Retry-After", delayLevels[currentDelayLevel] * 60 * 1000);
    ...
    if (!passwordMatches) {
      ...
      if (user.login_attempts < Number(process.env.MAX_LOGIN_ATTEMPTS) - 1) {
        ...
        await prisma.user.update({
          ...
          data: {
            login_attempts: user.login_attempts + 1,
            delay_level: user.delay_level + 1,
            last_attempt: new Date(), // Hora atual
          },
        });
      }
